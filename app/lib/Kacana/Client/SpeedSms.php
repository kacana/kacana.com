...
class SpeedSms
{
    ...
    private $ROOT_URL = KACANA_SPEED_SMS_URL;
    private $accessToken = KACANA_SPEED_SMS_ACCESS_TOKEN;
    ...
    public function sendSMS($to, $smsContent, $smsType = false, $sender = '') {
        if (!is_array($to) || empty($to) || empty($smsContent))
            return null;

        $type = SpeedSms::SMS_TYPE_CSKH;
        if ($smsType)
            $type = $smsType;
